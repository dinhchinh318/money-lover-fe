...
const AnalyticsLayout = () => {
  ...
  const navItems = useMemo(
    () => [
      { path: "/analytics", labelKey: "analytics.nav.diagnostic", fallback: "Phân tích Nguyên nhân", icon: Activity },
      { path: "/analytics/predictive", labelKey: "analytics.nav.predictive", fallback: "Dự đoán Tài chính", icon: LineChart },
      { path: "/analytics/prescriptive", labelKey: "analytics.nav.prescriptive", fallback: "Khuyến nghị Hành động", icon: Sparkles },
    ],
    []
  );

  // route đổi trên mobile -> đóng drawer
  useEffect(() => {
  ...
  const SidebarContent = ({ onNavigate }) => (
    <div className="p-4 space-y-2">
      {navItems.map((item) => (
        ...
        >
          <item.icon size={18} className={isActive(item.path) ? "text-[#0EA25E]" : "text-gray-500"} />
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col lg:flex-row">
      ...
      <div className="flex-1 overflow-auto w-full flex flex-col">
        ...
        <div className="lg:hidden bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-50">
          <h2 className="text-lg font-bold text-gray-800">{t("analytics.title", "Phân tích")}</h2>
          <button
            type="button"
            onClick={() => setMobileMenuOpen(true)}
            className="p-2 rounded-lg bg-gray-50 text-gray-600 hover:bg-gray-100 active:scale-95 transition-transform"
            aria-label={t("analytics.a11y.openMenu", "Mở menu")}
          >
            <MenuIcon size={24} />
          </button>
        </div>
      ...
      >
        ...
        >
          <div className="flex items-center justify-between p-4 border-b border-gray-200">
            <h3 className="font-bold text-gray-900">{t("analytics.drawerTitle", "Menu Phân tích")}</h3>
            <button
              type="button"
              onClick={closeMobileMenu}
              className="p-2 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors"
              aria-label={t("analytics.a11y.closeMenu", "Đóng menu")}
            >
              <X size={20} />
            </button>
          </div>
